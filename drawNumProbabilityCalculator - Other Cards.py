...
class possible_number_of_other_cards: # this is xn and pn bundled together
    def __init__(self, how_many:int, total_number_of_cards:int) -> None:
        # how many is the possible number of other cards
        ...
        # a) generate list with how_many * 'other' and total_number_of_cards - how_many * 'face'
        ...
        for permutation in perms(num_a=self.how_many, num_b=self.total_number_of_cards-self.how_many):
            ...
            for card in permutation:
               ...
               # print(f'card probability: {card_probability}')
                permutation_probability *= card_probability
               # print(f'permutation probability: {permutation_probability}')

                deck -= 1
                numbersOfCardsDict[f'num{card.capitalize()}'] -= 1
               # print(f"number of: {numbersOfCardsDict[f'num{card.capitalize()}']}")
    ...
    def __str__(self) -> str:
        return f'The probability of drawing {self.how_many} other cards when drawing {self.total_number_of_cards} total cards is {self.probability}'


if __name__ == "__main__":
    while True:

        print(possible_number_of_other_cards(how_many=1, total_number_of_cards=2))

        
        # this part calculates the probabilities
        workbook = openpyxl.load_workbook(filename='Draw_Number_Probabilities.xlsx')
        worksheet = workbook['Other_Sheet']

        possible_numbers_of_other_cards_dict = dict()

        for col in worksheet.iter_cols(min_row=2, min_col=2, max_col=17, max_row=17):
            for cell in col:
                if cell.value != 'N/A':
                    possible_numbers_of_other_cards_dict[f'{cell.column_letter}{cell.row}'] = possible_number_of_other_cards(how_many=worksheet[f"A{cell.row}"].value, total_number_of_cards=worksheet[f"{cell.column_letter}1"].value)
                    cell.value = possible_numbers_of_other_cards_dict[f'{cell.column_letter}{cell.row}'].probability
                    pass
                pass
            pass
        ...
        for col in worksheet.iter_cols(min_row=2, min_col=2, max_col=17, max_row=17):
            for cell in col:
                if cell.value != 'N/A':
                    averageDict[f'{num2words.num2words(worksheet[f"{cell.column_letter}1"].value)}Average'] += (possible_numbers_of_other_cards_dict[f'{cell.column_letter}{cell.row}'].probability * possible_numbers_of_other_cards_dict[f'{cell.column_letter}{cell.row}'].how_many)
