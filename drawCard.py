def draw( # this is the main function. it's best practice to program within a function rather than write everything globally. everything in this program is inside of draw().
    ...
    class Card:
        def __init__(self, card, numCard) -> None: # initializes Card class
            ...
            self.numCard = numCard # this attribute stores the number of each type of card in the deck, as shown on line 30
    ...
    joker, ace, king, queen, other = Card('joker', numJokers), Card('ace', numAces), Card('king', numKings), Card('queen', numQueens), Card('other', numOther)

    def drawOne(): # this function simulates drawing a card based on a random number and returns what card was drawn.
        nonlocal deck # this pulls the deck variable we created earlier (line 8) into this function, so it can be updated as cards are drawn
        ...
        # when the program finds which card the random number responds to, it increments numDrawn (line 17),
        # decrements numCard (line 16), and decrements deck (line 8). decrementing deck and numCard affects
        # the probability property (lines 19-21) of every card, simulating drawing from the deck without
        # replacement.

        for activeCard in [other, queen, king, ace, joker]: # the list is ordered like this because it corresponds to the graphic this way
            if drawNum <= activeCard.probability: # checks if the random number is in range of the card that's currently being checked.
                # if it is:
                deck -= 1                # decrements deck (line 8, line 33) to simulate removing a card from the deck.→ → → → → → → ↘      The deck and instance.numCard variables are necessary
        ...
        return activeCard # when called, drawOne will run and then act as the variable activeCard
...
# the following code is only run if drawCard.py is run as an independent python program:
